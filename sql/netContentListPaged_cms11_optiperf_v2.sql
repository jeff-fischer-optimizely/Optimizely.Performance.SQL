/*
    dbo.netContentListPaged_cms11_optiperf_v2        OPT-0011        risk: Low

    Derived from EPiServer.Cms.Core 11.21.5. CMS 11 only.

    OPT-0010's body plus OPTION (RECOMPILE) on the seven statements that read @ContentItems.
    Use below compatibility level 150; at 150 and above use OPT-0010, which is the same body
    without the hints. The two entries have disjoint windows and the capability probe picks.
    ...
    THE SECOND FIX: TABLE VARIABLE CARDINALITY
    ------------------------------------------
    @ContentItems is a table variable, so below compatibility level 150 it is estimated at
    one row no matter how many ids the caller packed. Seven statements read it -- the UPDATE
    that resolves language, the language list, the version list, the page data, the property
    load, the categories and the access rows -- and all seven get plans sized for a single
    page while the caller is loading a listing of them. The property load is the expensive
    one; a nested loop into tblContentProperty per page is fine for one page and not for two
    thousand.

    OPTION (RECOMPILE) compiles each against the row count actually present. It cannot change
    which rows a statement returns, and it does not disturb the @@ROWCOUNT tests that follow
    two of these SELECTs -- the hint is part of the same statement.

    WHY LOW RISK
    ------------
    Both changes are result-preserving by inspection: the unpack produces the same rows, and
    a recompile directive produces the same rows. Neither depends on the optimiser choosing
    well, only on it being given true information.

    Preconditions: maximumCompatibilityLevel 140.
*/

IF OBJECT_ID('dbo.netContentListPaged_cms11_optiperf_v2', 'P') IS NOT NULL
    DROP PROCEDURE dbo.netContentListPaged_cms11_optiperf_v2;
GO

CREATE PROCEDURE [dbo].[netContentListPaged_cms11_optiperf_v2]
(
