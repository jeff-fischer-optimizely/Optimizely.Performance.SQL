...
namespace Optimizely.Performance.SQL.V11.Tests
{
    ...
    /// <remarks>
    /// <para>
    /// Shared across the whole assembly because installing is a process-wide act: Harmony
    /// patches <c>SqlCommand</c> for everything running in the AppDomain, so there is no
    /// such thing as a per-test installation.
    /// </para>
    /// <para>
    ...
    {
        private const string Database = "OptiPerfV11Test";
        ...
        public ShimFixture()
        {
            _configurationDirectory = Path.Combine(Path.GetTempPath(), "optiperf-v11-" + Guid.NewGuid().ToString("n"));
        ...
        {
            Execute(MasterConnectionString, Drop());
            Execute(MasterConnectionString, "CREATE DATABASE [" + Database + "];");

            var connectionString = new SqlConnectionStringBuilder(MasterConnectionString) { InitialCatalog = Database }
                .ConnectionString;
...
END";
        ...
        private string BuildApprovedSql()
        {
            ...
            {
                GeneratedBy = "Optimizely.Performance.SQL.V11.Tests",
                Statements = new[]
                {
